...
const dashboard = async (req, res) => {
  try {
    ...
    try {
      ...
      response.userRevenue = userRevenueResult[0]?.total
        ? parseInt(userRevenueResult[0].total.toFixed(2))
        : 0;
      response.overallRevenue = overallRevenueResult[0]?.total || 0;
    } catch (error) {
      errors.revenueData = error.message;
    }
    ...
    try {
      ...
      const totalRecentOrders = recentOrders.reduce(
        (sum, day) => sum + day.count,
        0
      );
      const sellingRate =
        totalDays > 0 ? (totalRecentOrders / totalDays).toFixed(2) : 0;
      response.sellingRate = sellingRate;
    } catch (error) {
      errors.sellingRate = error.message;
    }
    ...
    try {
      ...
      const salesPerformanceData = {
        "7d": Array.from({ length: 7 }, (_, i) => {
          const dayStart = moment(weekRanges[weekRanges.length - 1].start)
            .add(i, "days")
            .startOf("day");
          ...
          const uniqueOrderIds = new Set(
            ordersForDay.map((o) => o.orderId.toString())
          );
        ...
        "30d": weekRanges.map((week, index) => {
          ...
          const uniqueOrderIds = new Set(
            ordersForWeek.map((o) => o.orderId.toString())
          );
      ...
      response.salesPerformance = salesPerformanceData;
    } catch (error) {
      errors.salesPerformance = error.message;
    }

    // 4. Get low stock products
    try {
      const matchStageforStock = isAdmin
        ? {}
      ...
      response.lowStockProducts = lowStockItems;
    } catch (error) {
      errors.lowStockProducts = error.message;
    }
    ...
    try {
      ...
      const ConfirmOrder = sellerOrders.filter(
        (v) =>
          // v.orderDetails.status != "pending" &&
          v.orderDetails.status != "cancelled"
      ).length;

      response.sellerOrders = sellerOrders;
      response.ConfirmOrder = ConfirmOrder;
    } catch (error) {
      errors.sellerOrders = error.message;
    }
    ...
    try {
      ...
      const categoryStats = categories.map((category) => {
        ...
        const salesPercentage =
          totalSales > 0 ? ((salesAmount / totalSales) * 100).toFixed(2) : 0;
      ...
      errors.categoryStats = error.message;
    }
    ...
    res.status(500).json({
      error: error.message,
      message:
        "An unexpected error occurred while processing the dashboard data",
    });
  }
};
